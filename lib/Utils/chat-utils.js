...
export const processSyncAction = (syncAction, ev, me, initialSyncOpts, logger) => {
    ...
    if (action?.muteAction) {
        ...
        // Worth surfacing: the NCT salt is server-issued and gates cstoken (and therefore the
        // 463 reach-out accounting). Accounts that have never been issued one carry no
        // nct_salt_sync mutation at all, so this line is the only positive signal it arrived.
        logger?.info({ operation, saltLen: action.nctSaltSyncAction.salt?.length }, 'nct_salt_sync received');
