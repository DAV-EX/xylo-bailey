...
export const addTransactionCapability = (state, logger, { maxCommitRetries, delayBetweenTriesMs }) => {
    ...
    return {
        ...
        isInTransaction,
        // Run `work` with the AsyncLocalStorage transaction context cleared, so its get/set
        // hit the underlying store directly. Used by fire-and-forget writes that run after the
        // enclosing transaction has committed (a plain set there would append to the already-
        // flushed ctx.mutations and never persist).
        exitTransaction: (work) => txStorage.exit(work),
